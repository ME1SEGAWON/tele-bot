...
global.loadDatabase = async function loadDatabase() {
  await global.db.read(); // Pastikan untuk membaca database
  global.db.data = global.db.data || { users: {} }; // Inisialisasi default jika tidak ada data
}
...
app.get('/api/getMoney', async (req, res) => {
    await loadDatabase(); // Pastikan database dimuat
    const userId = req.query.user;
    const user = db.data.users[userId];
    if (user) {
        res.json({ money: user.diamond, username: user.name });
    } else {
...
app.post('/api/verifyUser', async (req, res) => {
  ...
  if (user) {
      ...
      user.otp = otp;
      await db.write();
      // Logika untuk mengirim OTP ke pengguna melalui bot
      // Misalnya menggunakan method sendMessage jika Anda menggunakan bot Telegram
      await sendMessageToUser(userId, `Kode OTP Anda: ${otp}`);
...
app.post('/api/updateMoney', async (req, res) => {
    await loadDatabase(); // Pastikan database dimuat
    ...
    if (userData) {
        ...
        await db.write();
        res.json({ money: userData.diamond });
    } else {
        res.status(404).json({ error: 'User not found' });
    }
});
